...
if(!isset($_POST['start'])){
	$output = array(
			"status" => false,
			"error" => "Holiday Start Date Missing",
			"errorCode" => 103,
			"response" => ""
	);
	die(json_encode($output));
...
if(!isset($_POST['end'])){
	$output = array(
			"status" => false,
			"error" => "Holdiay End Date Missing",
			"errorCode" => 103,
			"response" => ""
	);
	die(json_encode($output));
...
$end = $_POST['end'];
	...
	mysql_query("INSERT INTO `d_holidays`( `title`, `start`, `end`) VALUES ('{$title}','{$start}','{$end}')");
