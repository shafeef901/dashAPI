...
$events_query = mysql_query("SELECT `title`, `date`, `timeFrom`, `timeTo` FROM `d_events` WHERE `status`=true");
$holidays_query = mysql_query("SELECT * FROM `d_holidays` WHERE 1");
...
while($holiday = mysql_fetch_assoc($holidays_query)){
		$status = true;
	 // to grant edit or delete status to owner
		$response [] = array(
				"title"=>$holiday['title'],
