...
$list = mysql_query("SELECT * FROM `d_classmasterlist` WHERE 1");
...
while($class = mysql_fetch_assoc($list)){
	 ...
	 $courseInfo = mysql_fetch_assoc(mysql_query("SELECT * FROM `dm_courseclassmapping` WHERE `class` = '{$class['class']}' && `division` = '{$class['division']}' && `day` = '{$day}' && ('{$t}' > `timeFrom` AND '{$t}' < `timeTo`)"));

	 $teacherInfo = mysql_fetch_assoc(mysql_query("SELECT * FROM `d_teachersmasterlist` WHERE `teacherId` = '{$courseInfo['teacherId']}' "));
