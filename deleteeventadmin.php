...
if(!isset($_POST['isRecurring'])){
	$output = array(
		"status" => false,
		"error" => "isRecurring Flag is not set",
		"errorCode" => 102,
		"response" => ""
	);
	die(json_encode($output));
...
{
	if($deleteAll == 0){
		mysql_query("DELETE FROM `d_events` WHERE `id`='{$id}' ");
		$response=array(
	...
	else if(($deleteAll == 1) && ($isRecurring == 1)){
		$recursionId = $_POST['recursionId'];
		mysql_query("DELETE FROM `d_events` WHERE `recursionId`='{$recursionId}' ");
